...
class DiscussionPollsModel extends Gdn_Model {
   ...
   * @param boolean $settings Added for GDN_Model::save compatibility
   */
   ...
   * @originalParameter int $PollID
   * @param array $where
   * @param array $options
   */
  public function Delete($where=[], $options=[]) {
    if (gettype($where) == "integer") {
      $this->OriginalDeleteMethod($where);
    } else {
      try {
        $this->Database->BeginTransaction();
   ...
   * @param int $PollID
   */
  public function OriginalDeleteMethod($PollID) {
    try {
      $this->Database->BeginTransaction();
      $this->SQL->Delete('DiscussionPolls', array('PollID' => $PollID));
      $this->SQL->Delete('DiscussionPollQuestions', array('PollID' => $PollID));
