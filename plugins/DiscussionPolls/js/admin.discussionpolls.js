...
jQuery(document).ready(function($) {
  ...
  if ($('#DP_Questions0').val()) {
    ...
    // THIS FOLLOWING IF PART IS ADDED TO ORIGINAL CODE @JANI
    // check attach poll if there is poll already made
    if ($('#DP_Questions01').val()) {
      $('#Form_DP_Attach').prop('checked', true);
    }
    else {
...
//    if (1==1) {
  ...
  $('#DP_AddOption').click(function(event) {
    ...
    if (ExistingPoll) {
      ...
      $(label).text(function() {
	if (optionNum > 10) {
          return $(this).text().replace(/(\d+)/, optionNum - 10 + 1); }
	else {
