...
if (!function_exists('writeComment')) :
     ...
     *
     * Prior to 2.1,
     * this also output the discussion ("FirstComment") to the browser.
     * That has moved to the discussion.php view.
     *
     * @param DataSet $comment .
     * @param Gdn_Controller $sender .
     * @param Gdn_Session $session .
     * @param int $CurrentOffet How many comments into the discussion we are
     * (for anchors).
     */
    function writeComment($comment, $sender, $session, $currentOffset) {
        ...
        <li class="<?php echo $cssClass; ?> pageBox" id="<?php echo 'Comment_'.$comment->CommentID; ?>">
            <div class="Comment">
                ...
                <?php
                // Write a stub for the latest comment
                // so it's easy to link to it from outside.
                if (
                    $currentOffset == Gdn::controller()->data('_LatestItem')
                    && Gdn::config('Vanilla.Comments.AutoOffset')
                ) {
                    ...
                    <div class="AuthorWrap">
                        ...
                        <span class="AuthorInfo">
                            <?php
                                if (
                                    Gdn::config(
                                        "Feature.CustomProfileFields.Enabled"
                                    )
                                    && function_exists('getCustomFields')
                                    && $author->UserID && !$author->Deleted
                                ) {
