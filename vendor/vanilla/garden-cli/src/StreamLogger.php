...
class StreamLogger implements LoggerInterface {
     ...
     *
     * This method takes either a format string for **strftime()**
     * or a callable that must format a timestamp.
     *
     * @param string|callable $format The new format.
     * @return $this
     * @see strftime() which was deprecated in PHP8 -> @see date()
     */
    public function setTimeFormat($format) {
        if (is_string($format)) {
            $this->timeFormatter = function ($t) use ($format): string {
                // return strftime($format, $t);
                $phpDateFormat = strftime_format_to_date_format($format);
                return date($phpDateFormat, $t);
            };
        } else {
