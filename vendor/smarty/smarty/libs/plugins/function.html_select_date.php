...
function smarty_function_html_select_date($params, Smarty_Internal_Template $template)
{
    ...
    $display_months = true;
    $display_years = true;
    $month_format = strftime_format_to_date_format('%B');
    /* Write months as numbers by default  GL */
    $month_value_format = strftime_format_to_date_format('%m');
    $day_format = '%02d';
            ...
            $_text = (
                isset($month_names)
                    ...
                    : (
                        ...
                        $month_format === 'm'
                            ? $_val
                            // : strftime($month_format, $_month_timestamps[ $i ])
                            : date($month_format, $_month_timestamps[$i])
                    )
              );
            $_value = (
                ...
                $month_value_format === 'm'
                    ? $_val
                    // : strftime($month_value_format, $_month_timestamps[ $i ])
                    : date($month_value_format, $_month_timestamps[$i])
            );
            $_html_months .= (
                '<option value="'
                . $_value
